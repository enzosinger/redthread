"""Probe-first telemetry collector.

Current runtime mainly records post-run and daemon probes, not all target traffic.
"""
...
class TelemetryCollector:
    """Active probe for canaries and optional organic records."""
    ...
    async def record_interaction(
        ...
        """Record one interaction and compute its response embedding."""
    ...
    async def inject_canary_batch(self, target: Any) -> list[TelemetryRecord]:
        """Send all canaries to the target and record the results."""
    ...
    def should_inject_canaries(self) -> bool:
        """Return True when a new canary batch should be injected."""
